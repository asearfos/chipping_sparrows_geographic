...
col_to_skip = ['CatalogNo', 'ComparedStatus', 'RecordingDay', 'RecordingMonth', 'RecordingTime', 'RecordingYear',
...
songVarTable_forPCA = StandardScaler().fit_transform(data_for_PCA.drop('FromDatabase', axis=1))
...
finalDf = pd.concat([data_for_PCA[['FromDatabase']], principalDf], axis=1)
print(pca.explained_variance_ratio_)
...
colors = {'eBird': '#1f78b4', 'Xeno-Canto': '#33a02c', 'MacaulayLibrary': '#542788', 'old': '#f17300'}
...
for group in finalDf.FromDatabase.unique():
    sdata = finalDf.loc[finalDf['FromDatabase'].isin([group])]
    PC1_mean = np.mean(sdata['PC1'])
    PC2_mean = np.mean(sdata['PC2'])
    cov = np.cov(sdata['PC1'], sdata['PC2'])
    ax0.scatter(sdata['PC1'], sdata['PC2'], color=colors.get(group), alpha=0.6, s=50, label=group, edgecolors=None,
                linewidth=0)
    # ax0.add_artist(e)
